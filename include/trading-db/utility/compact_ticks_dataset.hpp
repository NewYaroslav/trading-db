#pragma once
#ifndef TRADING_DB_UTILITY_COMPACT_TICKS_DATASET_HPP_INCLUDED
#define TRADING_DB_UTILITY_COMPACT_TICKS_DATASET_HPP_INCLUDED
	...
	namespace utility {
		...
		public:
			...
			inline void write_sequence(
				...
				double last_bid = 0;
				double last_ask = 0;
				uint64_t last_time_bid = 0;
				uint64_t last_time_ask = 0;
				double start_bid = 0;
				double start_ask = 0;
				...
				for (const auto &tick : ticks) {
					if (!std::isnan(tick.bid) && tick.bid != 0) {
						if (last_bid == 0) {
							start_bid = last_bid = tick.bid;
							start_time_bid = last_time_bid = tick.timestamp_ms;
							continue;
						}
				...
				for (const auto &tick : ticks) {
					if (!std::isnan(tick.ask) && tick.ask != 0) {
						if (last_ask == 0) {
							start_ask = last_ask = tick.ask;
							start_time_ask = last_time_ask = tick.timestamp_ms;
							continue;
						}
				...
				const uint64_t start_amplitude_bid = (uint64_t)((start_bid * (double)price_factor) + 0.5d);
				const uint64_t start_amplitude_ask = (uint64_t)((start_ask * (double)price_factor) + 0.5d);
				...
				const uint8_t reg_b0 = calc_uint_type(std::max(start_amplitude_bid, start_amplitude_ask));
				...
				const size_t length = (timestamp_day == 0 ? 4 : 2) +
...
#endif // TRADING_DB_UTILITY_COMPACT_TICKS_DATASET_HPP_INCLUDED
